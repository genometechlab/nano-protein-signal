...
cdef class LogisticDistribution(Distribution):
	"""
	A logistic distribution, which has heavier tails than a normal distribution.
	Useful for capturing outliers and extreme values in HMM states.
	"""

	def __init__(self, mu, s, frozen=False):
		"""
		Make a new Logistic distribution with location parameter mu and
		scale parameter s. The PDF is:
		...
		self.summaries = []
		self.name = "LogisticDistribution"
		self.frozen = frozen
	...
	cdef double _log_probability(self, double symbol):
		"""
		Calculate the log probability in optimized Cython.
		Using the formula: log(f(x)) = -(x-mu)/s - log(s) - 2*log(1 + exp(-(x-mu)/s))
		"""

		cdef double mu = self.parameters[0], s = self.parameters[1]
		cdef double z = -(symbol - mu) / s

		# For numerical stability, use different formulas for positive and negative z
		if z > 0:
			return z - clog(s) - 2 * clog(1 + cexp(-z))
		else:
			return -clog(s) - 2 * clog(cexp(z) + 1)

	def sample(self):
		"""
		Sample from this logistic distribution and return the value sampled.
		Using inverse transform sampling: X = mu + s * log(U/(1-U))
		where U ~ Uniform(0,1)
		"""
		...
		return self.parameters[0] + self.parameters[1] * math.log(u / (1 - u))

	def from_sample(self, items, weights=None, inertia=0.0, min_scale=0.01):
		"""
		Set the parameters of this Distribution to maximize the likelihood of
		the given sample. For logistic distribution, we use method of moments
		as MLE requires iterative optimization.

		Method of moments:
		- mu = sample mean
		- s = sqrt(3) * sample_std / pi
		"""
		...
		if len(weights[weights != 0]) > 1:
			...
			variance = numpy.dot((items - mean) ** 2, weights) / weights.sum()
			...
			# Only one data point, can't update scale
			std = self.parameters[1] * math.pi / math.sqrt(3)

		# Convert standard deviation to logistic scale parameter
		# For logistic distribution: Var = (pi^2 * s^2) / 3
		# Therefore: s = sqrt(3) * std / pi
		scale = std * math.sqrt(3) / math.pi

		# Enforce minimum scale
		scale = max(scale, min_scale)

		# Calculate the new parameters, respecting inertia
		prior_mu, prior_s = self.parameters
		self.parameters = [prior_mu * inertia + mean * (1 - inertia),
						   prior_s * inertia + scale * (1 - inertia)]
	...
	def summarize(self, items, weights=None):
		...
		# Calculate weights if none are provided
		if weights is None:
			weights = numpy.ones_like(items)
		else:
			weights = numpy.asarray(weights)
		...
		# For logistic distribution, we need to save the mean and variance
		mean = numpy.average(items, weights=weights)
		variance = numpy.dot((items - mean) ** 2, weights) / weights.sum()
	...
	def from_summaries(self, inertia=0.0, min_scale=0.01):
		'''
		Update parameters from the summaries collected.
		'''

		# If no summaries stored or the distribution is frozen, don't do anything
		if len(self.summaries) == 0 or self.frozen == True:
			return

		summaries = numpy.asarray(self.summaries)

		# Calculate the new mean and variance from summaries
		mean = numpy.average(summaries[:, 0], weights=summaries[:, 2])
		variance = numpy.sum([(v + m ** 2) * w for m, v, w in summaries]) \
				   / summaries[:, 2].sum() - mean ** 2
		...
		# Convert to logistic scale parameter
		scale = std * math.sqrt(3) / math.pi
